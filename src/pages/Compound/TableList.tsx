...
const ComponentCommon: React.FC<IComponentCommon> = ({
	keyObject,
	valObject,
}) => {
	return (
		<TableRow key={keyObject}>
			...
			<TableCell align="left">{valObject}</TableCell>
		</TableRow>
	);
};
	...
	return casNums.map((casNum, index) => {
		return (
			<TableRow key={casNum}>
				...
				<TableCell align="left">{casNum}</TableCell>
			</TableRow>
		);
	});
};
...
const ComponentComedogenicity: React.FC<IComponentComedogenicity> = ({
	keyObject,
	valObject,
}) => {
...
const TableList: React.FC<IProps> = ({ row }) => {
	...
	if (isRowComedogenicity)
		return (
			<ComponentComedogenicity keyObject={key} valObject={String(val)} />
		);
	if (!isRowCas && !isRowComedogenicity)
		return <ComponentCommon keyObject={key} valObject={val} />;
};
